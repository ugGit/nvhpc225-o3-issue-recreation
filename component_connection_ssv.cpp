...
#include "geometry/shared_definitions.hpp"
 ...
 * pattern. But this is something that surely can be tweaked.
 */
...
traccc::measurement_container_types::host component_connection_ssv(
    const traccc::cell_container_types::host& data) { // TODO remeoved const from function (because here it's not a memberfunction)
    ...
    */
     ...
     */
    traccc::measurement_container_types::host out;

    for (std::size_t i = 0; i < data.size(); ++i) {
        vecmem::vector<traccc::measurement> v;
        ...
        out.push_back(traccc::cell_module{0}, std::move(v));
    }
